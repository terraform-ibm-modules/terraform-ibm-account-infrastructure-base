...
variable "trusted_profile_unique_identifier" {
  type        = string
  description = "Unique string for the trusted profile policy. NOTE: The `trusted_profile_unique_identifier` variable is only used by terraform for building map objects, it is not use for any actual resource naming. Changing this value will cause resources to be recreated."
  default     = null
}
